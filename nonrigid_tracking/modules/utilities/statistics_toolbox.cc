...
float Sigma(const std::vector<float>& data){
    // 创建数据副本以避免修改原始数据
    std::vector<float> data_copy = data;
    float median = Median(data_copy);
    
    // 计算每个数据点与中位数的绝对偏差
    std::vector<float> absolute_deviations;
    absolute_deviations.reserve(data.size());
    for(float element : data){
        absolute_deviations.push_back(fabsf(element - median));
    }
    
    // 计算绝对偏差的中位数
    float mad = Median(absolute_deviations);
    
    // 将MAD转换为标准差的鲁棒估计
    // 对于正态分布，sigma ≈ 1.4826 * MAD
    const float scale_factor = 1.4826f;
