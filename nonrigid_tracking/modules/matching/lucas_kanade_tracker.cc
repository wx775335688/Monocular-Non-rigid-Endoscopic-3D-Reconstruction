...
int LucasKanadeTracker::Track(const Mat &newIm, std::vector<KeyPoint> &nextPts,
    ...
    // 初始化SSIM值存储
    last_ssim_values_.resize(nextPts.size(), 0.0f);
...
void LucasKanadeTracker::clear() {
    ...
    prevPts_.clear();
    last_ssim_values_.clear();
}

std::vector<float> LucasKanadeTracker::GetLastSSIMValues() const {
    return last_ssim_values_;
}
