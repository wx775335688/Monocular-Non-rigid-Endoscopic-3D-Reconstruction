...
void ImageVisualizer::DrawCurrentFrame(Frame &frame, const bool use_original_image) {
    ...
    // Display all keypoints with different colors based on their status
    // TRACKED_WITH_3D - Green
    vector<cv::KeyPoint> keypoints_3d = frame.GetKeypointsWithStatus({TRACKED_WITH_3D});
    for (int idx = 0; idx < keypoints_3d.size(); idx++) {
        DrawFeature(image_to_display, keypoints_3d[idx].pt, cv::Scalar(0, 255, 0)); // Green
    }

    // TRACKED - Yellow
    vector<cv::KeyPoint> keypoints_tracked = frame.GetKeypointsWithStatus({TRACKED});
    for (int idx = 0; idx < keypoints_tracked.size(); idx++) {
        DrawFeature(image_to_display, keypoints_tracked[idx].pt, cv::Scalar(0, 255, 255)); // Yellow
    }

    // JUST_TRIANGULATED - Blue
    vector<cv::KeyPoint> keypoints_triangulated = frame.GetKeypointsWithStatus({JUST_TRIANGULATED});
    for (int idx = 0; idx < keypoints_triangulated.size(); idx++) {
        DrawFeature(image_to_display, keypoints_triangulated[idx].pt, cv::Scalar(255, 0, 0)); // Blue
    }

    // CANDIDATE_3D - Cyan
    vector<cv::KeyPoint> keypoints_candidate = frame.GetKeypointsWithStatus({CANDIDATE_3D});
    for (int idx = 0; idx < keypoints_candidate.size(); idx++) {
        DrawFeature(image_to_display, keypoints_candidate[idx].pt, cv::Scalar(255, 255, 0)); // Cyan
    }

    // BAD - Red
    vector<cv::KeyPoint> keypoints_bad = frame.GetKeypointsWithStatus({BAD});
    for (int idx = 0; idx < keypoints_bad.size(); idx++) {
        DrawFeature(image_to_display, keypoints_bad[idx].pt, cv::Scalar(0, 0, 255)); // Red
    }

    // OUT_IMAGE_BOUNDARIES - Gray
    vector<cv::KeyPoint> keypoints_out = frame.GetKeypointsWithStatus({OUT_IMAGE_BOUNDARIES});
    for (int idx = 0; idx < keypoints_out.size(); idx++) {
        DrawFeature(image_to_display, keypoints_out[idx].pt, cv::Scalar(128, 128, 128)); // Gray
    }

    // BAD_FEATURE - Magenta
    vector<cv::KeyPoint> keypoints_bad_feature = frame.GetKeypointsWithStatus({BAD_FEATURE});
    for (int idx = 0; idx < keypoints_bad_feature.size(); idx++) {
        DrawFeature(image_to_display, keypoints_bad_feature[idx].pt, cv::Scalar(255, 0, 255)); // Magenta
    }

    // Add legend
    int legend_y = 30;
    cv::putText(image_to_display, "Legend:", cv::Point(10, legend_y), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255, 255, 255), 1);
    cv::putText(image_to_display, "Green: TRACKED_WITH_3D", cv::Point(10, legend_y + 20), cv::FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(0, 255, 0), 1);
    cv::putText(image_to_display, "Yellow: TRACKED", cv::Point(10, legend_y + 35), cv::FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(0, 255, 255), 1);
    cv::putText(image_to_display, "Blue: JUST_TRIANGULATED", cv::Point(10, legend_y + 50), cv::FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(255, 0, 0), 1);
    cv::putText(image_to_display, "Cyan: CANDIDATE_3D", cv::Point(10, legend_y + 65), cv::FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(255, 255, 0), 1);
    cv::putText(image_to_display, "Red: BAD", cv::Point(10, legend_y + 80), cv::FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(0, 0, 255), 1);
    cv::putText(image_to_display, "Gray: OUT_IMAGE_BOUNDARIES", cv::Point(10, legend_y + 95), cv::FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(128, 128, 128), 1);
    cv::putText(image_to_display, "Magenta: BAD_FEATURE", cv::Point(10, legend_y + 110), cv::FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(255, 0, 255), 1);
