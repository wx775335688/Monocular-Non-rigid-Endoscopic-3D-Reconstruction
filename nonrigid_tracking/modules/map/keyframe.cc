...
KeyFrame::KeyFrame(Frame &frame) {
    ...
    // Also recover just triangulated landmarks (convert them to TRACKED_WITH_3D for keyframe)
    vector<cv::KeyPoint> just_triangulated_keypoints = frame.GetKeypointsWithStatus({JUST_TRIANGULATED});
    vector<Eigen::Vector3f> just_triangulated_positions = frame.GetLandmarkPositionsWithStatus({JUST_TRIANGULATED});
    vector<absl::StatusOr<Eigen::Vector3f>> just_triangulated_gt = frame.GetGroundTruthWithStatus({JUST_TRIANGULATED});

    keypoints_.insert(keypoints_.end(), just_triangulated_keypoints.begin(), just_triangulated_keypoints.end());
    landmark_positions_.insert(landmark_positions_.end(), just_triangulated_positions.begin(), just_triangulated_positions.end());
    landmark_ground_truth_.insert(landmark_ground_truth_.end(), just_triangulated_gt.begin(), just_triangulated_gt.end());

    landmark_status_.resize(keypoints_.size());
    fill(landmark_status_.begin(), landmark_status_.end(), TRACKED_WITH_3D);

    vector<ID> mappoint_ids = frame.GetMapPointsIdsWithStatus({TRACKED_WITH_3D, JUST_TRIANGULATED});
