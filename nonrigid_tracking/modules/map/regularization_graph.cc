...
void RegularizationGraph::AddEdge(ID mappoint_id, ID mappoint_id_other, Eigen::Vector3f& relative_position) {
    // 获取MapPoint对象
    auto mappoint_1 = map_->GetMapPoint(mappoint_id);
    auto mappoint_2 = map_->GetMapPoint(mappoint_id_other);
    
    // 检查点是否存在
    if (!mappoint_1 || !mappoint_2) {
        return;
    }
    
    // 限制新点的邻居数量
    if (graph_.contains(mappoint_id)) {
        int current_neighbors = graph_[mappoint_id].size();
        if (current_neighbors >= mappoint_1->GetMaxNeighbors()) {
            // 已经达到最大邻居数量，不添加新边
            return;
        }
    }
    
    if (graph_.contains(mappoint_id_other)) {
        int current_neighbors_other = graph_[mappoint_id_other].size();
        if (current_neighbors_other >= mappoint_2->GetMaxNeighbors()) {
            // 已经达到最大邻居数量，不添加新边
            return;
        }
    }
    ...
    // 动态th_dist: 如果启用，使用平均距离*2作为阈值
    float th_dist = options_.weight_sigma;
    if (options_.use_dynamic_th_dist) {
        float avg_dist = ComputeAvgPointDist();
        th_dist = 2.0f * avg_dist;
        // 更新weight_sigma以反映动态阈值
        if (distance > th_dist) {
            // 距离太远，不添加边
            return;
        }
    ...
    // 计算基础权重
    float base_weight = InterpolationWeight(distance, options_.weight_sigma);
    
    // 应用初期权重因子，降低新点的权重
    float weight_factor = std::min(mappoint_1->GetInitialWeightFactor(), mappoint_2->GetInitialWeightFactor());
    edge->weight = base_weight * weight_factor;
...
std::shared_ptr<RegularizationGraph::Edge> RegularizationGraph::GetEdge(ID mappoint_id, ID mappoint_id_other) {
    // 修复hash bug：使用at()访问，避免自动插入默认值
    if (graph_.contains(mappoint_id) && graph_[mappoint_id].contains(mappoint_id_other)) {
        return graph_.at(mappoint_id).at(mappoint_id_other);
    }
    return nullptr;
}
...
std::vector<std::pair<ID, std::shared_ptr<RegularizationGraph::Edge>>>
    ...
    // Check if the mappoint_id exists in the graph
    if (!graph_.contains(mappoint_id)) {
        return std::vector<std::pair<ID, std::shared_ptr<RegularizationGraph::Edge>>>();
    }
...
int RegularizationGraph::UpdateVertex(ID mappoint_id) {
    ...
    Eigen::Vector3f landmark_position = mappoint->GetLastWorldPosition();
    ...
    for (auto & [other_mappoint_id, edge] : connections){
        ...
        auto other_mappoint = map_->GetMapPoint(other_mappoint_id);
        if (!other_mappoint) {
            continue; // Skip if other map point is invalid
        }
        
        Eigen::Vector3f other_landmark_position = other_mappoint->GetLastWorldPosition();
...
// 计算平均点距离（用于动态th_dist）
float RegularizationGraph::ComputeAvgPointDist() const {
    if (graph_.empty()) {
        return 1.0f;  // 默认值
    }
    ...
    for (const auto& [id, connections] : graph_) {
        for (const auto& [other_id, edge] : connections) {
            if (id < other_id) {  // 避免重复计算
                total_dist += edge->first_distance;
                count++;
            }
        }
    }
