...
Map::Map(Map::Options &options) : options_(options) {
    ...
    regularization_graph_options.streching_th = 2.0f;  // 放宽拉伸阈值，适应内窥镜变形
...
std::shared_ptr<KeyFrame> Map::GetNextUnmappedKeyFrame() {
    ...
    if (unmapped_keyframes_.empty()) {
        ...
        auto keyframe_id = unmapped_keyframes_.front();
        unmapped_keyframes_.pop_front();

        std::shared_ptr<KeyFrame> keyframe = nullptr;
        if (keyframes_.count(keyframe_id) > 0) {
            keyframe = keyframes_[keyframe_id];
        }
...
void Map::SetLastFrame(std::shared_ptr<Frame> frame, const std::vector<float>& ssim_values) {
    last_frame_mutex_.Lock();

    last_frame_ = frame;

    temporal_buffer_->InsertSnapshotFromFrame(*last_frame_, ssim_values);
...
bool Map::IsEarlyMapStage() {
    // 定义早期地图阶段：关键帧数量少于20或MapPoint数量少于500
    return GetKeyFrameCount() < 20 || GetMapPointCount() < 500;
}
