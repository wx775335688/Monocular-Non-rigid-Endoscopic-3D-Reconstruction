...
public:
    ...
    struct Snapshot {
        ...
        float deformation_magnitude;
        
        // Optional per-keypoint SSIM confidence values
        absl::flat_hash_map<int, float> keypoint_ssim;
    };

    void InsertSnapshotFromFrame(Frame& frame);
    // New overload: optionally provide per-keypoint SSIM values (same order as frame keypoints)
    void InsertSnapshotFromFrame(Frame& frame, const std::vector<float>& ssim_values);

    absl::btree_map<ID, Snapshot>& GetRawBuffer();

    std::vector<int> GetTriangulationCandidatesIds();

    int TrackLength(const int keypoint_id);
