...
public:
    ...
    bool GetEvaluationEnabled();

    // Tracking parameters
    int GetKLTWindowSize();
    int GetKLTMaxLevel();
    int GetKLTMaxIterations();
    float GetKLTEpsilon();
    float GetKLTMinEigThreshold();
    float GetKLTMinSSIM();

    // Stereo matcher parameters
    int GetStereoKLTWindowSize();
    int GetStereoKLTMaxLevel();
    int GetStereoKLTMaxIterations();
    float GetStereoKLTEpsilon();
    float GetStereoKLTMinEigThreshold();
    float GetStereoKLTMinSSIM();
...
private:
    ...
    int klt_max_iters_;
    float klt_epsilon_;
    float klt_min_eig_th_;
    float klt_min_ssim_;

    // Stereo matcher parameters
    int stereo_klt_window_size_;
    int stereo_klt_max_level_;
    int stereo_klt_max_iters_;
    float stereo_klt_epsilon_;
    float stereo_klt_min_eig_th_;
    float stereo_klt_min_ssim_;
